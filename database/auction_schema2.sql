...
CREATE TABLE IF NOT EXISTS `mydb`.`Auction` (
  ...
  PRIMARY KEY (`AuctionID`, `Seller_User_UserID`, `Rating_RatingID`),
...
CREATE TABLE IF NOT EXISTS `mydb`.`ShoppingCart` (
  ...
  `Buyer_Auction_AuctionID` INT(20) UNSIGNED NOT NULL,
  `Buyer_User_StatusID` INT UNSIGNED NOT NULL,
  PRIMARY KEY (`CartID`, `Buyer_User_UserID`, `Buyer_Auction_AuctionID`, `Buyer_User_StatusID`),
  UNIQUE INDEX `CardID_UNIQUE` (`CartID` ASC) VISIBLE,
  INDEX `fk_ShoppingCart_Buyer1_idx` (`Buyer_User_UserID` ASC, `Buyer_Auction_AuctionID` ASC, `Buyer_User_StatusID` ASC) VISIBLE,
  CONSTRAINT `fk_ShoppingCart_Buyer1`
    FOREIGN KEY (`Buyer_User_UserID` , `Buyer_Auction_AuctionID` , `Buyer_User_StatusID`)
    REFERENCES `mydb`.`Buyer` (`User_UserID` , `Auction_AuctionID` , `User_StatusID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `mydb`.`Bid` (
  ...
  `Auction_AuctionID` INT(20) UNSIGNED NOT NULL,
  `Buyer_User_UserID` INT(20) UNSIGNED NOT NULL,
  PRIMARY KEY (`BidID`, `Auction_AuctionID`, `Buyer_User_UserID`),
